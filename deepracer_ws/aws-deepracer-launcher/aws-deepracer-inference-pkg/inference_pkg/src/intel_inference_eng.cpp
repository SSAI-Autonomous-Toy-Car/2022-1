...
namespace {
	...
	std::string user_path = "/home/deepracer/deepracer_lidar/deepracer.xml";
	//change artifactPath to user_path
        auto network = core.ReadNetwork(user_path);
        ...
        */
        ...
        for (const auto& pair : network.getOutputsInfo()) {
            RCLCPP_INFO(inferenceNode->get_logger(), "_______________________________ output __________________________________________/n");
            RCLCPP_INFO(inferenceNode->get_logger(), "%s/n",pair.first);
            RCLCPP_INFO(inferenceNode->get_logger(), "__________________________________________________________________________________/n");
        }
        auto outputInfo = network.getOutputsInfo().find("continuous_actions")->second;
     ...
     template<typename T, typename V> void left1DImg(V *inputPtr,
                                                     cv::Mat &retImg,
        ...
        std::ifstream input;
        input.open("/home/deepracer/deepracer_lidar/leftImage.txt");
	    ...
	    for (int w = 0; w < width; w++) 
		    {
		    	std::string line;
		    	getline(input, line);
		        inputPtr[h * width + w] = std::stof(line);
		    }
        }     	
	...
	/*
        for (int  h = 0; h < height; h++) {
            for (int w = 0; w < width; w++) {
                inputPtr[h * width + w] = retImg.at<T>(h, w)[c]/255;
            }
        }
        */
     }
     
     template<typename T, typename V> void right1DImg(V *inputPtr,
                                                     cv::Mat &retImg,
                                                     std::shared_ptr<InferTask::ImgProcessBase> imgProcessPtr,
                                                     const sensor_msgs::msg::Image &imgData,
                                                     const std::unordered_map<std::string, int> &params,
        ...
        int width = retImg.cols;
        std::ifstream input;
        input.open("/home/deepracer/deepracer_lidar/rightImage.txt");
	...
	for (int  h = 0; h < height; h++) {
		    for (int w = 0; w < width; w++) 
		    {
		    	...
		    	getline(input, line);
		        inputPtr[h * width + w] = std::stof(line);
		        //RCLCPP_INFO(inferenceNode->get_logger(), "line : %f\n", std::stof(line));
		    }
        }     	
	...
	/*
        for (int  h = 0; h < height; h++) {
            for (int w = 0; w < width; w++) {
                inputPtr[h * width + w] = retImg.at<T>(h, w)/255.0;
            }
        }
        */
     }
     ...
     template<typename T, typename V> void loadStereoImg(V *inputPtr,
        ...
        for (int c = 0; c < channel; c++) {
            for (int  h = 0; h < height; h++) {
                for (int w = 0; w < width; w++) {
                    inputPtr[c * width * height + h * width + w] = retImg.at<T>(h, w)[c]/255;
                }
            }
        }
     }
...
namespace IntelInferenceEngine {
     ...
     : doInference_(false)
    {
        ...
        //change IntelInferenceEngine::RLInferenceModel::sensorCB to IntelInferenceEngine::RLInferenceModel::sensorCB_user
        sensorSub_ = inferenceNode->create_subscription<deepracer_interfaces_pkg::msg::EvoSensorMsg>(sensorSubName, 10, std::bind(&IntelInferenceEngine::RLInferenceModel::sensorCB_user, this, std::placeholders::_1));
    ...
    //user-defined sensorCB for unity model load and infer
    //user-defined sensorCB for unity model load and infer
    //user-defined sensorCB for unity model load and infer
    //user-defined sensorCB for unity model load and infer
    //user-defined sensorCB for unity model load and infer
    //user-defined sensorCB for unity model load and infer
    //user-defined sensorCB for unity model load and infer
    void RLInferenceModel::sensorCB_user(const deepracer_interfaces_pkg::msg::EvoSensorMsg::SharedPtr msg){
    	if(!doInference_){
    	    return;
    	}
    	try{
    	    for (size_t i = 0; i < inputNamesArr_.size(); ++i){
    	    	...
    	    	cv::Mat retData;
    	    	RCLCPP_INFO(inferenceNode->get_logger(), "sensorCB_user");
    	    	if(inputNamesArr_[i] == "obs_0"){
    	    	    RCLCPP_INFO(inferenceNode->get_logger(), "left camera");
    	    	    load1DImg<uchar, float>(inputPtr, retData, imgProcess_, msg->images[0], paramsArr_[i]);
    	    	}
    	    	else if(inputNamesArr_[i] == "obs_1"){
    	    	    RCLCPP_INFO(inferenceNode->get_logger(), "right camera");
    	    	    load1DImg<uchar, float>(inputPtr, retData, imgProcess_, msg->images[1], paramsArr_[i]);
    	    	}
    	    	else if(inputNamesArr_[i] == "obs_2"){
    	    	    RCLCPP_INFO(inferenceNode->get_logger(), "lidar");
    	    ...
    	    auto output = inferRequest_.GetBlob(outputName);
            ...
            RCLCPP_INFO(inferenceNode->get_logger(), "Values : [%f, %f]\n", outputData[0], outputData[1]);
            ...
            for (size_t label = 0; label < outputDims[1]; ++label) {
                ...
                RCLCPP_INFO(inferenceNode->get_logger(), "%f probability : %f \n",label, outputData[label]);
